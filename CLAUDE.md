...
### AD-08：FocusWeb dev/E2E 必须用 :3000（CORS 白名单）
**原因**：后端 CORS `allowed-origins` 仅 `http://localhost:3000,http://localhost:8081`。dev server 跑别的端口（如 :3001），浏览器 `Origin` 经 Vite 代理透传给后端，Spring CORS 判 403，mock 登录失败停在 /login（曾因此误判半天）。
**方案**：dev/E2E 固定 :3000；若被占（如 open-webui 自动重启抢端口），先停它再 `npm run dev -- --strictPort`，跑完恢复。生产同源（nginx 反代 `/api`）无此问题。

### AD-09：写前端 API 调用前先读 `context/api-contracts.md`
...
- 测试：`npm test`（Vitest 单元 7）、`npm run test:e2e`（Playwright **17 用例**，覆盖 7 页 + 交互 + 鉴权 + 响应式 + 键盘；`globalSetup` 跑前自动清 todos/goals/categories；选择器须容忍 antd 按钮汉字间距，用 `tests/e2e/helpers.ts` 的 `cjk()`）；默认 dev :3000，或设 `E2E_BASE_URL`
